...
The decoder structure must be identical for both Fast and Slow memory systems, mapping the $C$ bucket dimensions back to the original $n$ sequence-length structure.
...
### Token Features (`token_features_dim = 4`)
...
4. `memory_type`: A signed scalar indicating whether the token originated from the Fast (GPU) buffer or Slow (CPU) buffer ($\pm 1.0$).
...
- **Transformer Encoder Baseline:** The 4 continuous scalar features map via a simple raw linear projection `Linear(4, D_model)`, flowing through $N$ feedforward parameterised sub-layers before un-pooling.
