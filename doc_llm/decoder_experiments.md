...
The system employs a Transformer spatial decoder tasked with inverting the **linear-probing-based amplitude pooling** table: features mapped from 1024 sequence positions compressed down to 128 buckets. The model is forced to perform continuous regression (finding the input wave signal) while simultaneously performing discrete modular arithmetic (using the bucket index and Distance-to-Initial-Bucket `DIB`).

## Finding 1: The 8% Accuracy Plateau
During initial training runs, the model plateaued at approximately 8% accuracy.
- **Root Cause**: The network successfully learned the base modulo-based hash routing function ($Position \pmod{128}$). Since sequence length $n=1024$ and capacity $C=128$, there are $1024 / 128 = 8$ sequence positions that map to any given bucket modulo.
...
The model mathematically processes everything it needs to break this tie: it has the full continuous **signed** amplitude and the discrete bucket index, which together uniquely define the position. However, fusing continuous regression with integer modular geometry is notoriously difficult for transformers and requires deep algorithmic grokking to emerge.
