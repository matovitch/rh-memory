"""Evaluate a hard decoder scatter baseline on synthetic harmonic signals.

This script leaves the training pipeline untouched. It loads a frozen surrogate and
soft-distilled decoder, builds decoder input tokens from the surrogate, takes a
hard argmax over decoder logits for each bucket, then scatters each bucket's soft
amplitude back to the predicted source coordinate.
"""
...
def decoder_hard_scatter_baseline(
    decoder: RHDecoder,
    ...
    """Scatter decoder-token soft amplitudes at argmax decoder source positions."""
    logits = decoder(decoder_tokens)[:, : decoder_tokens.size(1), :n]
    predicted_slot = logits.argmax(dim=-1)
    source_index_by_slot = perm_1d.to(device=logits.device, dtype=torch.long)
    predicted_source = source_index_by_slot[predicted_slot]

    soft_amplitude = decoder_tokens[..., 0]
    baseline = torch.zeros(decoder_tokens.size(0), n, dtype=decoder_tokens.dtype, device=decoder_tokens.device)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Evaluate hard decoder scatter RelL2 on harmonic batches.")
...
def main() -> None:
    ...
    print(
        f"Decoder hard scatter baseline | device={device} | N={config.n} C={config.C} "
    ...
    with torch.no_grad():
        for batch_idx in range(1, args.batches + 1):
            sample = next(stream)
            baseline, _predicted_slot, predicted_source, doubt = decoder_hard_scatter_baseline(
                decoder,
    ...
    print("Summary:")
    print(f"  Decoder hard scatter RelL2 mean: {rel_l2_all.mean().item():.2f}%")
    print(f"  Decoder hard scatter RelL2 p50 : {rel_l2_all.median().item():.2f}%")
    print(f"  Decoder hard scatter RelL2 p90 : {rel_l2_all.quantile(0.90).item():.2f}%")
