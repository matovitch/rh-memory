"""Evaluate a soft surrogate scatter baseline on synthetic harmonic signals.

This script leaves the training pipeline untouched. It loads a frozen surrogate,
builds soft decoder tokens from surrogate logits, converts surrogate logits to
probabilities, then scatters each bucket's soft amplitude over all source
coordinates according to the surrogate probability mass.
"""
...
def surrogate_soft_scatter_baseline(
    surrogate_logits: torch.Tensor,
    decoder_tokens: torch.Tensor,
    perm_1d: torch.Tensor,
    temperature: float,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """Scatter surrogate-token soft amplitudes over every source position by surrogate probabilities."""
    baseline, probs = decoder_soft_scatter(surrogate_logits, decoder_tokens[..., 0], perm_1d, temperature)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Evaluate soft surrogate scatter RelL2 on harmonic batches.")
...
def main() -> None:
    ...
    print(
        f"Surrogate soft scatter baseline | device={device} | N={config.n} C={config.C} "
    ...
    with torch.no_grad():
        for batch_idx in range(1, args.batches + 1):
            sample = next(stream)
            baseline, doubt, support = surrogate_soft_scatter_baseline(
    ...
    print("Summary:")
    print(f"  Surrogate soft scatter RelL2 mean: {rel_l2_all.mean().item():.2f}%")
    print(f"  Surrogate soft scatter RelL2 p50 : {rel_l2_all.median().item():.2f}%")
    print(f"  Surrogate soft scatter RelL2 p90 : {rel_l2_all.quantile(0.90).item():.2f}%")
