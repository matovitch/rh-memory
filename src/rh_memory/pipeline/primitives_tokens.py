...
def reconstructor_tokens_from_decoder_logits_soft(
    x_perm: Float[Tensor, "B N"],
    perm_1d: Tensor,
    ...
    *,
    ...
    """
    Build differentiable reconstructor tokens from decoder logits.

    Output channels are ``[soft_amplitude, soft_normalized_source_index, decoder_doubt]``.
    """
    if temperature <= 0:
        raise ValueError(f"temperature must be positive, got {temperature}")
    ...
    if x_perm.shape != (B, n):
        raise ValueError(f"x_perm must have shape (B={B}, N={n}), got {tuple(x_perm.shape)}")
    if perm_1d.shape != (n,):
        raise ValueError(f"perm_1d must have shape (N={n},), got {tuple(perm_1d.shape)}")

    probs = F.softmax(decoder_logits / temperature, dim=-1)
    soft_amplitude = torch.einsum("bcn,bn->bc", probs, x_perm)
    source_idx = perm_1d.to(device=decoder_logits.device, dtype=decoder_logits.dtype)
    soft_source_idx = torch.einsum("bcn,n->bc", probs, source_idx)
    soft_normalized_source_index = soft_source_idx / float(max(1, n - 1))
    decoder_doubt = normalized_entropy(probs)
    return torch.stack([soft_amplitude, soft_normalized_source_index, decoder_doubt], dim=-1)


def reconstructor_tokens_from_decoder_logits_hard(
    x_perm: Float[Tensor, "B N"],
    perm_1d: Tensor,
    decoder_logits: Float[Tensor, "B C N"],
    *,
    bucket_amplitude: Float[Tensor, "B C"] | None = None,
    temperature: float = 1.0,
) -> Float[Tensor, "B C 3"]:
    """
    Build reconstructor tokens with hard argmax source positions from decoder logits.

    Output channels are ``[bucket_amplitude, hard_normalized_source_index, decoder_doubt]``.
    When ``bucket_amplitude`` is omitted, the amplitude is gathered from ``x_perm`` at the hard slot.
    """
    if temperature <= 0:
        raise ValueError(f"temperature must be positive, got {temperature}")
    B, C, n = decoder_logits.shape
    ...
    if perm_1d.shape != (n,):
        raise ValueError(f"perm_1d must have shape (N={n},), got {tuple(perm_1d.shape)}")
    if bucket_amplitude is not None and bucket_amplitude.shape != (B, C):
        raise ValueError(f"bucket_amplitude must have shape ({B}, {C}), got {tuple(bucket_amplitude.shape)}")

    j_star = decoder_logits.argmax(dim=-1)
    if bucket_amplitude is None:
        amplitude = torch.gather(x_perm, 1, j_star)
    else:
        amplitude = bucket_amplitude.to(device=decoder_logits.device, dtype=decoder_logits.dtype)

    source_idx_by_slot = perm_1d.to(device=decoder_logits.device, dtype=decoder_logits.dtype)
    source_idx = source_idx_by_slot[j_star]
    normalized_source_index = source_idx / float(max(1, n - 1))
    probs = F.softmax(decoder_logits / temperature, dim=-1)
    decoder_doubt = normalized_entropy(probs)
    return torch.stack([amplitude, normalized_source_index, decoder_doubt], dim=-1)
