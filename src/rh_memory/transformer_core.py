...
class MultiheadAttentionCore(nn.Module):
    ...
    def _validate_mask(self, attn_mask: Tensor, q_len: int, kv_len: int) -> None:
    ...
    def forward(self, x_q: Tensor, x_kv: Tensor | None = None, attn_mask: Tensor | None = None) -> Tensor:
        if self.mode == "cross":
            if x_kv is None:
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x: Tensor, x_kv: Tensor | None = None, attn_mask: Tensor | None = None) -> Tensor:
        x_norm = self.norm1(x)
        if self.mode == "cross":
...
class TransformerCrossBlock(nn.Module):
    ...
    def forward(self, q: Tensor, memory: Tensor, self_attn_mask: Tensor | None = None) -> Tensor:
